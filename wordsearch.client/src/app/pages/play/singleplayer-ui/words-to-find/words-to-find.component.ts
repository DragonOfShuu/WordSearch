...
export class WordsToFindComponent {
  ...
  wordsInfo = signal<null|{wordsFound: WordDictionary, wordsToFind: string[]}>(null)
  ...
  constructor() {
    this.toFindChanged$.subscribe((value) => {
      ...
      if (wordInfo && wordInfo.wordsToFind.every((word) => this.wordsToFind().includes(word))) {
        const found = this.wordsFound()
        this.wordsInfo.set(found === null ? null : {wordsToFind: value, wordsFound: found});
        return;
      }
